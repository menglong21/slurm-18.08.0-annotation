...
main (int argc, char **argv)
{
	 ...
	 */
	cc = sysconf(_SC_OPEN_MAX);//获取进程默认可以打开的最大文件描述符数
	for (i = 3; i < cc; i++)
		close(i);
	 ...
	 */
	if (geteuid() == 0) {//geteuid()用来取得执行目前进程有效的用户识别码
	...
	if (_slurmd_init() < 0) {//2.slurmd初始化,重要，没有追完
		error( "slurmd initialization failed" );
		fflush( NULL );//fflush()会强迫将缓冲区内的数据写回参数stream指定的文件中，如果参数stream为NULL，fflush()会将所有打开的文件数据更新。
		exit(1);
	}
	//3.验证uid
	...
	//4.设置信号
	xsignal(SIGTERM, slurmd_shutdown);//管理节点发过来REQUEST_REBOOT_NODES类型消息，slurmd_req执行_rpc_reboot会发出信号SIGTERM
...
　　//3、若父进程退出导致进程组成为孤儿进程组，且该进程组中有进程处于停止状态（收到SIGSTOP或SIGTSTP信号），该信号会被发送到该进程组中的每一个进程。
	...
	xsignal_block(blocked_signals);//线程屏蔽
	...
	if ((oom_value = getenv("SLURMD_OOM_ADJ"))) {
		...
		set_oom_adj(i);//设置进程保活优先级。NATIVE_ADJ 为-17，则程序为native级别永驻内存
	}

	_kill_old_slurmd();//5.杀死当前运行的slurmd

	if (conf->mlock_pages) {//命令行参数M
		/*
	...
	/*
	 * Restore any saved revoked credential information还原任何已保存的已撤销的凭据信息
	 */
	 ///opt/slurm17/spool/slurmd/cred_state
	if (!conf->cleanstart && (_restore_cred_state(conf->vctx) < 0))
		return SLURM_FAILURE;
	//6.初始化各种插件
	if (jobacct_gather_init() != SLURM_SUCCESS)//JobAcctGatherType=jobacct_gather/linux
		fatal("Unable to initialize jobacct_gather");
	if (job_container_init() < 0)//#JobContainerType=job_container/none，JobContainerType应用于作业分配，而ProctrackType应用于作业步骤
		fatal("Unable to initialize job_container plugin.");
	if (container_g_restore(conf->spooldir, !conf->cleanstart))//恢复作业容器状态
		error("Unable to restore job_container state.");
	if (core_spec_g_init() < 0)//CoreSpecPlugin	标识用于执行核心专门化的插件。
		fatal("Unable to initialize core specialization plugin.");
	if (switch_g_node_init() < 0)//	SwitchType 交换或互联类型
		fatal("Unable to initialize interconnect.");
	if (node_features_g_init() != SLURM_SUCCESS)//NodeFeaturesPlugins用于英特尔KNL处理器，KNL家族已终结，该项无用
		fatal("failed to initialize node_features plugin");
	if (conf->cleanstart && switch_g_clear_node_state())//清理网络互联状态
		fatal("Unable to clear interconnect state.");
	switch_g_slurmd_init();//src/plugin/switch/,基本没用
	file_bcast_init();//初始化file_bcast_list

	_create_msg_socket();//7.创建一个socket连接，socket文件描述符保存在conf->lfd

	conf->pid = getpid();//获取进程描述符
	pidfd = create_pidfile(conf->pidfile, 0);//创建pid文件

	rfc2822_timestamp(time_stamp, sizeof(time_stamp));//获取系统时间并格式化
	...
	run_script_health_check();//9.执行health check脚本

	msg_aggr_sender_init(conf->hostname, conf->port,//10.初始化msg_collection并创建数据聚合发送线程
			     conf->msg_aggr_window_time,
			     conf->msg_aggr_window_msgs);

	slurm_thread_create_detached(NULL, _registration_engine, NULL);//11.向管理节点注册计算节点

	_msg_engine();//12.启动消息引擎（死循环），为每一个连接建立一个线程，处理接收到的消息
...
{
	_increment_thd_count();//增加活跃线程计数

	while (!_shutdown) {
		if ((sent_reg_time == (time_t) 0) &&
		    (send_registration_msg(SLURM_SUCCESS, true) !=//发送注册消息
		     SLURM_SUCCESS)) {
			debug("Unable to register with slurm controller, "
			      "retrying");
		} else if (_shutdown || sent_reg_time) {//注册成功或者被shutdown才退出
			break;
		}
		sleep(1);
	}

	_decrement_thd_count();//减少活跃线程计数
	return NULL;
}
...
_msg_engine(void)
{
	...
	msg_pthread = pthread_self();//1.获得线程自身PID
	slurmd_req(NULL);	/* 2.初始化定时器 */
	while (!_shutdown) {//3.如果接收到信号SIGTERM和SIGINT，则置位_shutdown
		if (_reconfig) {//4.如果接收到信号SIGHUP，则置位_reconfig
			verbose("got reconfigure request");
			_wait_for_all_threads(5); /* Wait for RPCs to finish */
			_reconfigure();//重新配置
		}
		if (_update_log)//5.如果接收到信号SIGUSR2，则置位_reconfig
			_update_logging();//更新log信息
		cli = xmalloc (sizeof (slurm_addr_t));
		if ((sock = slurm_accept_msg_conn(conf->lfd, cli)) >= 0) {//6.接收一个新连接
			_handle_connection(sock, cli);//7.处理新连接
			continue;
		}
		/*
...
static void
_decrement_thd_count(void)//减少活跃线程计数
{
	slurm_mutex_lock(&active_mutex);
	if (active_threads > 0)
		active_threads--;//活跃线程数减少
	slurm_cond_signal(&active_cond);
	slurm_mutex_unlock(&active_mutex);
}

static void
_increment_thd_count(void)//增加活跃线程计数
{
	bool logged = false;
...
_wait_for_all_threads(int secs)
{
	...
	while (active_threads > 0) {
		...
		rc = pthread_cond_timedwait(&active_cond, &active_mutex, &ts);//阻塞，等待超时
		if (rc == ETIMEDOUT) {
			error("Timeout waiting for completion of %d threads",
			      active_threads);
			slurm_cond_signal(&active_cond);//激活
			slurm_mutex_unlock(&active_mutex);
			return;//循环退出
		}
	}
...
static void _handle_connection(int fd, slurm_addr_t *cli)
{
	...
	_increment_thd_count();//增加活跃线程计数
	slurm_thread_create_detached(NULL, _service_connection, arg);//创建连接服务线程_service_connection
}
...
_service_connection(void *arg)
{
	...
	slurm_msg_t_init(msg);//1.初始化消息
	if ((rc = slurm_receive_msg_and_forward(con->fd, con->cli_addr, msg, 0))//2.监听socket,接收并解析消息存在msg，需要转发的则转发
	   != SLURM_SUCCESS) {
...
cleanup:
	...
	xfree(con);
	slurm_free_msg(msg);
	_decrement_thd_count();//4.减少活跃线程计数
	return NULL;
}

static void _handle_node_reg_resp(slurm_msg_t *resp_msg)//处理消息返回值回调函数
{
	int rc;
...
send_registration_msg(uint32_t status, bool startup)
{
	...
	_fill_registration_msg(msg);//填充节点注册msg
	msg->status  = status;
	...
	if (conf->msg_aggr_window_msgs > 1) {
		...
		if (slurm_send_recv_controller_msg(&req, &resp_msg,//向管理节点发送计算节点注册消息
						   working_cluster_rec) < 0) {
			error("Unable to register: %m");
			ret_val = SLURM_FAILURE;
			goto fail;
		}
		slurm_free_node_registration_status_msg(msg);//释放msg

		_handle_node_reg_resp(&resp_msg);//处理消息返回值回调函数
...
_fill_registration_msg(slurm_node_registration_status_msg_t *msg)
{
	...
	gres_info = init_buf(1024);//获取gres信息，打包
...
_process_cmdline(int ac, char **av)
{
	...
	while ((c = getopt(ac, av, GETOPT_ARGS)) > 0) {//getopt解析参数
		switch (c) {
		case 'b':
			conf->boot_time = 1;
			break;
		...
		case 'D':
			...
			break;
		case 'f':
			conf->conffile = xstrdup(optarg);//配置文件
			break;
		case 'h':
			_usage();
			exit(0);
		...
		case 'L':
			...
			break;
		case 'M':
			conf->mlock_pages = 1;//mlockall函数将调用进程的全部虚拟地址空间加锁。防止出现内存交换，将该进程的地址空间交换到外存上。
			break;
		case 'n':
...
_create_msg_socket(void)
{
	...
	fd_set_close_on_exec(ld);//设置close_on_exec

	conf->lfd = ld;//将打开的文件描述符保存到conf中
...
_slurmd_init(void)
{
	...
	/*
	 * 首先处理命令行参数，因为其中一个选项可能是slurm配置文件的替代位置。
	 */
	 ...
	 * 这是拓扑堆栈所需要的。
	 * 节点表设置必须在_read_config()之前，这样才能设置正确的主机名。
	 */
	...
	//gres插件初始化,两个函数都要执行，gpu相关，重要
	if ((gres_plugin_init() != SLURM_SUCCESS) ||
	    (gres_plugin_node_config_load(cpu_cnt, conf->node_name, NULL)
	     != SLURM_SUCCESS))
		return SLURM_FAILURE;
	if (slurm_topo_init() != SLURM_SUCCESS)//拓扑网络初始化
		return SLURM_FAILURE;

	/*
	 * Get and set slurmd 拓扑 information
...
_restore_cred_state(slurm_cred_ctx_t ctx)
{
	...
	xstrcat(file_name, "/cred_state");// /opt/slurm17/spool/slurmd/cred_state
...
{
	if (signum == SIGTERM || signum == SIGINT) {//如果接收到信号SIGTERM和SIGINT，则置位_shutdown
		_shutdown = 1;
		if (msg_pthread && (pthread_self() != msg_pthread))
			pthread_kill(msg_pthread, SIGTERM);//向指定线程msg_pthread发送信号SIGTERM
...
static void _update_logging(void)
{
	...
	/*
	 * 将reconfig发送到每个stepd，以便它们也可以rotate。
	 */
