...
slurmd_req(slurm_msg_t *msg)
{
	...
	if (msg == NULL) {
		if (startup == 0)
			startup = time(NULL);//初始化时间startup
	...
	case REQUEST_RECONFIGURE:
		...
		last_slurmctld_msg = time(NULL);
		break;
	case REQUEST_REBOOT_NODES://重启节点
...
_rpc_reboot(slurm_msg_t *msg)
{
	...
	uid_t req_uid = g_slurm_auth_get_uid(msg->auth_cred,//munge认证
					     conf->auth_info);
	int exit_code;
	...
	if (!_slurm_authorized_user(req_uid))
		...
		if (reboot_program) {
			...
			reboot_msg = (reboot_msg_t *) msg->data;//reboot消息
			...
			else if ((exit_code = system(cmd)))//执行重启程序
			...
			/*
			 * 显式地关闭slurmd。
			 * 这通常是通过调用reboot_program来处理的，
			 * 但如果无法关闭该节点，那么至少会脱机该节点，直到人为进行干预。
			 */
...
//	struct stat	stat_buf;
	...
	if (_step_is_starting(req->job_id, NO_VAL)) {
		...
		if (_wait_for_starting_step(req->job_id, NO_VAL)) {//等待执行作业
			/*
...
_remove_starting_step(uint16_t type, void *req)
{
	...
	slurm_cond_broadcast(&conf->starting_steps_cond);//通知启动
fail:
	return rc;
}
