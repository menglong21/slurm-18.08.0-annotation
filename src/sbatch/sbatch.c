...
int main(int argc, char **argv)
{
	...
	pack_argv = argv;
	//解析剩余参数，生成job_req_list(异构作业)或desc（单个作业）,设置环境变量
	for (pack_inx = 0; !pack_fini; pack_inx++) {
		bool more_packs = false;
		init_envs(&pack_env);
		//解析剩余参数（1.脚本2.环境变量3.命令行），例如-M,opt.clusters
		...
		//用opt结构体填充作业描述符job_desc_msg_t（slurm.h）
	 ...
	 * and run it there
	 */
	 //-M,此时会初始化working_cluster_rec
	 //后面slurm_send_recv_controller_msg提交作业会跟这个cluster建立通讯
	if (opt.clusters) {
		if (job_req_list) {
			rc = slurmdb_get_first_pack_cluster(job_req_list,//获得运行异构作业的第一个集群
					opt.clusters, &working_cluster_rec);
		} else {
			rc = slurmdb_get_first_avail_cluster(desc,//获得将运行作业的第一个集群
					opt.clusters, &working_cluster_rec);
		}
		if (rc != SLURM_SUCCESS) {
	...
	while (true) {
		...
		if (job_req_list)
			...
			rc = slurm_submit_batch_job(desc, &resp);//发出RPC以提交作业以供稍后执行
		if (rc >= 0)
			break;
