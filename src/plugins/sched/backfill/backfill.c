...
static void _load_config(void)
{
	...
	sched_params = slurm_get_sched_params();//获得调度参数
	debug_flags  = slurm_get_debug_flags();
	//下面解析调度参数，然后设置相应的全局变量
	...
	if (sched_params && (tmp_ptr = strstr(sched_params, "bf_window="))) {//回填窗口
...
/* backfill_agent - 分离线程周期性的尝试回填作业detached thread periodically attempts to backfill jobs */
...
#endif
	_load_config();//从配置文件中解析调度参数，设置相应全局变量
...
next_task:
 ...
 TRY_LATER:
		...
		if ((job_ptr->start_time <= now) &&
			...
			rc = _start_job(job_ptr, resv_bitmap);//启动作业
...
skip_start:
	...
	_pack_start_test(node_space);//测试启动作业，合适就启动
...
static void _pack_kill_now(pack_job_map_t *map)
{
	...
	while ((rec = (pack_job_rec_t *) list_next(iter))) {
		...
		last_job_update     = now;
		build_cg_bitmap(job_ptr);//构建完成此作业的节点的位图
		job_completion_logger(job_ptr, false);//调用job_completion_logger，记录作业数据
		deallocate_nodes(job_ptr, false, false, false);//释放节点
		/*
		...
		if (!job_ptr->node_bitmap_cg ||
			...
			batch_requeue_fini(job_ptr); //完成所有步骤后，完成批处理作业重新排队逻辑，以便后续作业显示在单独的会计记录中
	}
	list_iterator_destroy(iter);
}
...
static void _pack_start_test(node_space_map_t *node_space)
{
	...
	while ((map = (pack_job_map_t *) list_next (iter))) {
		...
		rc = _pack_start_now(map, node_space);//启动pack作业
		if (rc != SLURM_SUCCESS) {
			if (debug_flags & DEBUG_FLAG_HETERO_JOBS) {
