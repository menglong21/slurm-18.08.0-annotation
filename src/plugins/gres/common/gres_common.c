...
extern int common_node_config_load(List gres_conf_list,//传入参数，配置好的全局链表gres_conf_list
				   char *gres_name,
				   List *gres_devices)//传出参数，gres设备链表
{
	...
	itr = list_iterator_create(gres_conf_list);//遍历全局链表gres_conf_list
	while ((gres_slurmd_conf = list_next(itr))) {
		...
		while ((one_name = hostlist_shift(hl))) {//遍历主机名列表,生成gres_devices链表
			if (!*gres_devices)
				*gres_devices =
					list_create(destroy_gres_device);

			gres_device = xmalloc(sizeof(gres_device_t));
			list_append(*gres_devices, gres_device);//添加gres_devices
			//填充gres_device
