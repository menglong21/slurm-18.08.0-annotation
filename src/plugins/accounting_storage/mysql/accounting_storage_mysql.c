	...
	bool rollback, char *cluster_name)
{
	...
	if (!(mysql_conn = create_mysql_conn(//初始化mysql_conn结构体
	...
	errno = SLURM_SUCCESS;
	mysql_db_get_db_connection(mysql_conn, mysql_db_name, mysql_db_info);//获得有效连接
	...
	if (mysql_conn->rollback) {
		if (!commit) {
			...
			if (rc != SLURM_SUCCESS) {
				...
				if (mysql_db_commit(mysql_conn))//提交
					error("commit failed");
			}
		}
	}
