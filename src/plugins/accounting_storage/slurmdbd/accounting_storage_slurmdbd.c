...
static void *_set_db_inx_thread(void *no_data)
{
	...
	while (!plugin_shutdown) {
		 ...
		 * later in this function. */
		 //在这里，我们已经从作业提交中卸载了数据库中的启动作业。
		//这使得提交作业的速度更快，而且不会锁定等待数据库返回数据库索引的控制器。
		//尽管有可能修改这里的作业db_index，但我们使用读锁，因为数据不是那么敏感，只会在稍后的函数中更新。
		...
		while ((job_ptr = list_next(itr))) {
			...
			if (!IS_JOB_UPDATE_DB(job_ptr)) {
				 ...
				 * it accordingly.
				 */
				//在这里，我们将db_index设置为no_VAL 64，
				//以避免出现潜在的竞争条件，此时作业只能运行，
				//并且在对dbd的调用返回之前，作业启动并需要发送开始消息，
				//但如果db_index为0，则不会发送，从而导致有关分配的信息丢失。
				//将其设置为no_val将通知DBD这种情况，它将相应地处理它。
	...
	end_it:
		...
		*/
		//因为不用花很多时间来做这个检查每5秒做一次。
		//这对DBD很有帮助，所以它不需要找到没有启动rpc的工作的db_indexs。
		gettimeofday(&tvnow, NULL);
		abs.tv_sec = tvnow.tv_sec + 5;
		abs.tv_nsec = tvnow.tv_usec * 1000;
		//阻塞等待条件变量db_inx_cond（fini触发）或者超时
...
extern int init ( void )
{
	if (first) {
		...
		if (job_list && !(slurm_get_accounting_storage_enforce() &
			 ...
			 * (in the slurmctld) */
			 //只有在定义了JOB_LIST时才这样做
			 //创建_set_db_inx_thread线程，重置数据库中的作业索引
					    ...
					    slurmdb_job_cond_t *job_cond)
{
	...
	req.data = &get_msg;
	rc = send_recv_slurmdbd_msg(SLURM_PROTOCOL_VERSION, &req, &resp);//实际通信过程
