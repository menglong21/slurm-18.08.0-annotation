...
//打开一个跟slurmdbd的连接并且设置slurmdbd_conn
	...
	while (*slurmdbd_conn->shutdown == 0) {
		...
		if (agent_list) {
			...
			if (cnt > handle_agent_count) {
				...
				buffer = pack_slurmdbd_msg(//打包将要发给dbd的信息
				...
				buffer = (Buf) list_peek(agent_list);//从agent_list上取一个包下来
		} else
			buffer = NULL;
		slurm_mutex_unlock(&agent_lock);
		if (buffer == NULL) {
		 ...
		 * complete. */
		rc = slurm_persist_send_msg(slurmdbd_conn, buffer);//向slurmdbd发送信息包buffer
		if (rc != SLURM_SUCCESS) {
 ...
 *
 * Returns SLURM_SUCCESS or an error code */
 //发送RPC给slurmdbd,不等待回应。如果DBD没有响应，RPC将被排队稍后处理
extern int send_slurmdbd_msg(uint16_t rpc_version, slurmdbd_msg_t *req)
{
	...
	buffer = slurm_persist_msg_pack(//打包将要发送给dbd的信息
	...
	if (cnt < max_agent_queue) {
		if (list_enqueue(agent_list, buffer) == NULL)//将buffer加到agent_list上，通知agent发送
