...
int main(int argc, char **argv)
{
	...
	slurm_conf_init(NULL);//解析配置文件
	sacct_init();//初始化一些数据结构
	parse_command_line(argc, argv);//解析命令行参数
	...
	switch (op) {
	case SACCT_LIST:
		print_fields_header(print_fields_list);//打印输出表头
		if (get_data() == SLURM_ERROR)//从存储获取作业信息,通信过程
			exit(errno);
		if (params.opt_completion)
			do_list_completion();//作业完成信息，依赖于作业完成配置项
		else
			do_list();//正常打印
		break;
	case SACCT_HELP:
