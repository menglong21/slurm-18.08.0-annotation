...
extern int get_data(void)//获取数据
{
	...
	if (params.opt_completion) {
		...
		jobs = slurmdb_jobs_get(acct_db_conn, job_cond);//获取信息jobs
	}
...
extern void parse_command_line(int argc, char **argv)
{
	...
	static struct option long_options[] = {//命令行参数对应关系
	...
	while (1) {		/* now cycle through the command line */
		...
		case OPT_LONG_NAME:
			...
			break;
		case 'o':
			xstrfmtcat(params.opt_field_list, "%s,", optarg);//-o,指定
			break;
		case 'p':
