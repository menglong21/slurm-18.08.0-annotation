				      ...
				      mysql_db_info_t *db_info)
{
	...
	if (!(mysql_conn->db_conn = mysql_init(mysql_conn->db_conn))) {
		...
		mysql_options(mysql_conn->db_conn, MYSQL_OPT_RECONNECT,
			      &reconnect);
#endif
		//2.设置额外的连接选项，并影响连接的行为
		mysql_options(mysql_conn->db_conn, MYSQL_OPT_CONNECT_TIMEOUT,
			      (char *)&my_timeout);
		while (!storage_init) {
			if (!mysql_real_connect(mysql_conn->db_conn, db_host,//3.创建连接
				...
				if (err == ER_BAD_DB_ERROR) {
					debug("Database %s not created.  "
					      "Creating", db_name);
					rc = _create_db(db_name, db_info);//4.创建数据库
				} else {
					...
					mysql_close(mysql_conn->db_conn);//5.关闭连接
				 ...
				 storage_field_t *fields, char *ending)
{
	 ...
	 */
	//我们有一个名为table_defs_table的内部表，它包含数据库中每个表的定义。
	//为了加快速度，我们只需要检查一下，看看是否有什么变化。
