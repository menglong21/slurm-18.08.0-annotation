...
int main(int argc, char **argv)
{
	...
	//从slurmdbd.conf文件中加载slurmdbd配置，并保存在slurmdbd_conf
	if (read_slurmdbd_conf())
		exit(1);
	//解析命令行参数
	_parse_commandline(argc, argv);
	//基于配置参数重置slurmdbd日志记录
	_update_logging(true);
	//升级nice
	_update_nice();
	//杀死当前运行的slurmdbd
	_kill_old_slurmdbd();
	//设置守护进程（-D设置foreground）
	if (foreground == 0)
		_daemonize();
	...
	//获取存储端口db_conn,此处调用插件
	//C:\SugonHPC\slurm-18.08.0\src\plugins\accounting_storage\mysql\accounting_storage_mysql.c
	...
	if (reset_lft_rgt) {
		int rc;
		if ((rc = acct_storage_g_reset_lft_rgt(//重置assoc表
	...
	while (1) {
		if (slurmdbd_conf->dbd_backup &&//备用dbd
			...
			run_dbd_backup();//以备用控制器运行
			if (!shutdown_time)
				assoc_mgr_refresh_lists(db_conn, 0);
		} else if (slurmdbd_conf->dbd_host &&//主dbd
...
static int _send_slurmctld_register_req(slurmdb_cluster_rec_t *cluster_rec)
{
	...
	if (fd < 0) {
		...
		//3.封一包数据ACCOUNTING_REGISTER_CTLD
		slurm_msg_t out_msg;
...
static void _become_slurm_user(void)
{
	...
	//setuid和setgid的作用是让执行该命令的用户以该命令拥有者的权限去执行，
