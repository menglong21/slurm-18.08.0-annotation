...
int main(int argc, char **argv)
{
	...
	int option_index;
	static struct option long_options[] = {//选项结构体
		{"all",      0, 0, 'a'},
		{"cluster",  1, 0, 'M'},
		{"clusters", 1, 0, 'M'},
		{"details",  0, 0, 'd'},
	...
	while (1) {
		...
		case (int)'a':
			...
			break;
		case (int)'d':
			detail_flag++;//detail此处影响show输出运行作业的每个节点上的核心数
			break;
		case (int)'F':
			future_flag = 1;
			break;
	...
	if (input_field_count)
		...
		error_code = _get_command (&input_field_count, input_fields);//接收二级命令

	while (error_code == SLURM_SUCCESS) {
		error_code = _process_command (input_field_count,//处理命令,核心
					       input_fields);
...
_print_ping (void)
{
	...
	_ping_slurmctld(control_cnt, control_machine);//ping slurmctld控制器
...
_ping_slurmctld(uint32_t control_cnt, char **control_machine)
{
	...
	for (i = 0; i < control_cnt; i++) {
		int status = 0;
		if (slurm_ping(i) == SLURM_SUCCESS)//实际ping
			status = 1;
		else
			down_msg = true;
		if (i == 0)
...
static int _process_command (int argc, char **argv)
{
	...
	if (xstrncasecmp(tag, "abort", MAX(tag_len, 5)) == 0) {
		...
		detail_flag = 1;//detail此处影响show输出运行作业的每个节点上的核心数
	}
...
static void _show_it(int argc, char **argv)
{
	...
	if (xstrncasecmp(tag, "aliases", MAX(tag_len, 1)) == 0) {
		...
		scontrol_print_node_list (val);//打印节点信息
