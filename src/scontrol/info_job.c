...
scontrol_load_job(job_info_msg_t ** job_buffer_pptr, uint32_t job_id)
{
	...
	//根据old_job_info_ptr和job_id情况加载作业信息结构体
	if (old_job_info_ptr) {
	...
	if (error_code == SLURM_SUCCESS) {
		...
		*job_buffer_pptr = job_info_ptr;//指针指向结果
	}
...
extern void scontrol_print_job(char * job_id_str)
{
	...
	error_code = scontrol_load_job(&job_buffer_ptr, job_id);//加载作业信息
	if (error_code) {
		exit_code = 1;
		if (quiet_flag != 1)
			slurm_perror ("slurm_load_jobs error");
	...
	for (i = 0, job_ptr = job_buffer_ptr->job_array;//遍历作业链表
		...
		slurm_print_job_info(stdout, job_ptr, one_liner);//打印作业信息，scontrol show job jobid --detail
		if (save_array_str) {
