...
static msg_aggr_t *_handle_msg_aggr_ret(uint32_t msg_index, bool locked)
{
	...
	while ((msg_aggr = list_next(itr))) {
		...
		if (!msg_index) {
			...
			list_remove(itr);
		} else if (msg_aggr->msg_index == msg_index) {//逐个删除msg_aggr
			list_remove(itr);
			break;
		}
...
static int _send_to_next_collector(slurm_msg_t *msg)
{
	...
	if (msg_collection.debug_flags & DEBUG_FLAG_ROUTE)
		info("msg aggr: send_to_next_collector: getting primary next "
		     "collector");
	if ((next_dest = route_g_next_collector(&i_am_collector))) {//获得路由新地址
		if (msg_collection.debug_flags & DEBUG_FLAG_ROUTE) {
			char addrbuf[100];
			slurm_print_slurm_addr(next_dest, addrbuf, 32);
		...
		memcpy(&msg->address, next_dest, sizeof(slurm_addr_t));//拷贝地址
		rc = slurm_send_only_node_msg(msg);//将msg发送到新地址
	}

	if (!next_dest || (rc != SLURM_SUCCESS))
		rc = _send_to_backup_collector(msg, rc);//发给备用collector
...
static void * _msg_aggregation_sender(void *arg)
{
	...
	slurm_cond_signal(&msg_collection.cond);//激活

	while (msg_collection.running) {
		/* Wait for a new msg to be collected */
		slurm_cond_wait(&msg_collection.cond, &msg_collection.mutex);//阻塞，等待激活
		...
		slurm_cond_timedwait(&msg_collection.cond,
				     &msg_collection.mutex, &timeout);//阻塞，等待激活或超时
		...
		slurm_msg_t_init(&msg);//打包msg
		msg.msg_type = MESSAGE_COMPOSITE;
		msg.protocol_version = SLURM_PROTOCOL_VERSION;
		msg.data = &cmp;

		if (_send_to_next_collector(&msg) != SLURM_SUCCESS) {//将msg发给下一个collector
		...
		slurm_cond_broadcast(&msg_collection.cond);//恢复msg收集
	}
				 ...
				 uint64_t max_msg_cnt)
{
	...
	slurm_thread_create(&msg_collection.thread_id,//创建msg聚合发送线程
...
extern void msg_aggr_sender_fini(void)
{
	...
	slurm_cond_signal(&msg_collection.cond);//唤醒某阻塞线程继续执行
	slurm_mutex_unlock(&msg_collection.mutex);

	pthread_join(msg_collection.thread_id, NULL);//以阻塞的方式等待指定的线程msg_collection.thread_id结束
			     ...
			     void (*resp_callback) (slurm_msg_t *msg))
{
	...
	slurm_mutex_lock(&msg_collection.mutex);
	if (msg_collection.max_msgs == true) {//max_msgs下面置位
		slurm_cond_wait(&msg_collection.cond, &msg_collection.mutex);
	}
	...
	list_append(msg_collection.msg_list, msg);//将msg添加到消息收集器

	count = list_count(msg_collection.msg_list);

	/* First msg in collection; initiate new window */
	if (count == 1)
		slurm_cond_signal(&msg_collection.cond);//激活消息收集发送线程
	...
	if (wait) {
		...
		if (pthread_cond_timedwait(&msg_aggr->wait_cond,
					   ...
					   &timeout) == ETIMEDOUT)
			_handle_msg_aggr_ret(msg_aggr->msg_index, 1);//删除msg_addr
		wait_count--;
		slurm_mutex_unlock(&msg_collection.aggr_mutex);
