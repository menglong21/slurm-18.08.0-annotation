...
plugin_load_from_file(plugin_handle_t *p, const char *fq_path)
{
	...
	*p = PLUGIN_INVALID_HANDLE;
	...
	/*
	 *  Check for file existence and access permissions检查文件存在和可获取许可
	 */
	 ...
	 *
	 */
	plug = dlopen(fq_path, RTLD_LAZY);//打开动态库
	if (plug == NULL) {
		     ...
		     const char *names[], void *ptrs[])
{
	...
	while (so_name[i]) {//gres_gpu.so/gres_nic.so/gres_mic.so
		if (so_name[i] == '/')
			so_name[i] = '_';
		i++;
	}
	if (!(dir_array = slurm_get_plugin_dir())) {//获取插件目录
	...
	for (i = 0; ; i++) {
		...
		file_name = xstrdup_printf("%s/%s", head, so_name);//拼接插件完整路径
		...
		if ((stat(file_name, &st) < 0) || (!S_ISREG(st.st_mode))) {
			...
			if ((err = plugin_load_from_file(&plug, file_name))//从动态库文件中加载插件并初始化，返回插件句柄plug
			   == EPLUGIN_SUCCESS) {
				if (plugin_get_syms(plug, n_syms,//根据传入的方法集合动态解析插件到ptrs，先加载后解析
						    names, ptrs) >= n_syms) {
					debug3("Success.");
					xfree(file_name);
					break;
...
//果然用了dlsym,开源视频服务器软件mjpg-streamer里面用过
		 ...
		 void *ptrs[] )
{
	...
	count = 0;
	for ( i = 0; i < n_syms; ++i ) {
		ptrs[ i ] = dlsym( plug, names[ i ] );//根据传入的name解析插件内部的函数,通过ptrs带出来，这个方法巧妙的实现了动态解析。
		if ( ptrs[ i ] )
			++count;
		else
