 ...
 */
//将有关作业的信息加载到存储中，通常是在作业开始执行时，但可能更早。
					    ...
					    slurmdb_job_cond_t *job_cond)
{
	...
	if (slurm_acct_storage_init(NULL) < 0)
		return NULL;
	ret_list = (*(ops.get_jobs_cond))(db_conn, uid, job_cond);//as_mysql_jobacct_process_get_jobs,src\plugins\accounting_storage\mysql\as_mysql_jobacct_process.c
