...
/*
 * 调用以检查节点是否支持设置CPU频率
 * 如果是，在cpu_freq_data结构中初始化字段
 * cpufreq内核子系统
 * https://www.ibm.com/developerworks/cn/linux/l-cn-cpufreq/
...
cpu_freq_init(slurmd_conf_t *conf)
{
	...
	if (!cpufreq) {
		...
		for (cpuidx = 0; cpuidx < cpu_freq_count; cpuidx++)
			_cpu_freq_init_data(cpuidx);//逐个初始化cpufreq数组
	}
	...
	for (i = 0; i < cpu_freq_count; i++) {
		snprintf(path, sizeof(path),
			 PATH_TO_CPU
			 "cpu%u/cpufreq/scaling_available_governors", i);//构建路径
		if ((fp = fopen(path, "r")) == NULL)//打开文件
			continue;
		if (fgets(value, LINE_LEN, fp) == NULL) {//读取value
			fclose(fp);
			continue;
		}
		//逐个判断是否存在
		if (strstr(value, "conservative")) {//保守动态调整
		...
		if (strstr(value, "performance")) {//高性能
		...
		if (strstr(value, "powersave")) {//省电模式
		...
		if (strstr(value, "userspace")) {//用户控制，cpufrequtils 
