...
static slurm_protocol_config_t *_slurm_api_get_comm_config(void)
{
	...
	//用conf的port和host[0]填充controller_addr
	...
	//将controller_addr拷贝到proto_conf->controller_addr[0]
	memcpy(&proto_conf->controller_addr[0], &controller_addr,
	       sizeof(slurm_addr_t));
	//用conf的port和其余host填充其余proto_conf->controller_addr
 ...
 */
 //是否通过associations记账
 //association指的是关联关系或者层级关系，
 //指的其实就是cluster到account到subaccount到user这么种层次关系，
 //是一种结构上的限制
...
int slurm_init_msg_engine_addrname_port(char *addr_name, uint16_t port)
{
	...
	slurm_setup_sockaddr(&addr, port);//初始化sockaddr_in地址

	return slurm_init_msg_engine(&addr);//创建socket,设置，绑定，监听
}
 ...
 */
 //在bsd套接字实现中，它创建了一个SOCK_STREAM套接字，并在其上调用connect，
 //一个名为SOCK_DGRAM套接字定义为仅接收来自connect调用slurm_address的地址/端口对参数的消息
 //目前它实际上只是一个sockaddr_in
 ...
 * RET slurm_fd	- file descriptor of the connection created
 */
 //调用connect使无连接的数据报连接主或二级slurmctld消息引擎
				      ...
				      slurmdb_cluster_rec_t *comm_cluster_rec)
{
	...
	//根据working_cluster_rec是否设置，打开不同连接，返回不同fd
	if (!comm_cluster_rec) {//working_cluster_rec没有设置
		/* This means the addr wasn't set up already */
		if (!(proto_conf = _slurm_api_get_comm_config()))//根据配置文件获取通信数据结构
	...
	for (retry = 0; retry < max_retry_period; retry++) {
		if (retry)
			sleep(1);
		if (comm_cluster_rec) {//如果working_cluster_rec存在，打开对应socket连接的fd
		...
		} else if (proto_conf->vip_addr_set) {//如果proto_conf->vip_addr_set存在，打开对应socket连接的fd
			fd = slurm_open_msg_conn(&proto_conf->vip_addr);
			if (fd >= 0)
				goto end_it;
			debug("Failed to contact controller: %m");
		} else {//打开proto_conf->controller_addr对应socket连接的fd
			if (!*use_backup) {
...
int slurm_receive_msg(int fd, slurm_msg_t *msg, int timeout)
{
	...
	if (msg->conn) {
		...
		buffer = slurm_persist_recv_msg(msg->conn);//1.长连接接收数据
		if (!buffer) {
		...
		if (rc) {
			...
			slurm_persist_conn_close(msg->conn);//3.关闭连接
			return SLURM_ERROR;
		}
				  ...
				  slurm_msg_t *msg, int timeout)
{
...
#if	_DEBUG
	_print_data (buf, buflen);//调试，输出数据
#endif
	buffer = create_buf(buf, buflen);//2.创建buffer,将buf数据存入

	if (unpack_header(&header, buffer) == SLURM_ERROR) {//3.解压缩一个slurm协议头
		free_buf(buffer);
		rc = SLURM_COMMUNICATIONS_RECEIVE_ERROR;
		goto total_return;
	}
	//4.解析包头
	if (check_header_version(&header) < 0) {//检查协议版本
		slurm_addr_t resp_addr;
		char addr_str[32];
		int uid = _unpack_msg_uid(buffer);
	...
	if (header.forward.cnt > 0) {
		...
		if (forward_msg(msg->forward_struct, &header) == SLURM_ERROR) {//转发
	...
	if ( (header.body_length > remaining_buf(buffer)) ||
	     (unpack_msg(msg, buffer) != SLURM_SUCCESS) ) {//根据消息类型解析消息主体
		(void) g_slurm_auth_destroy(auth_cred);
		free_buf(buffer);
		rc = ESLURM_PROTOCOL_INCOMPLETE_PACKET;
		goto total_return;
 ...
 * RET int 		- returns 0 on success, -1 on failure and sets errno
 */
 //打开到控制器的连接，向控制器发送消息，侦听响应，然后关闭连接
				...
				slurmdb_cluster_rec_t *comm_cluster_rec)
{
	...
	slurmdb_cluster_rec_t *save_comm_cluster_rec = comm_cluster_rec;//working_cluster_rec
...
tryagain:
	...
	while (retry) {
		 ...
		 */
		retry = 0;
		rc = _send_and_recv_msg(fd, request_msg, response_msg, 0);//发送接收数据
 ...
 */
//使用扇出向指定的节点列表发送消息。然后返回包含类型(ret_data_info_t)的列表
