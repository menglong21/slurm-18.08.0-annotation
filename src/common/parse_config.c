 ...
 */
static int _conf_hashtbl_index(const char *key)//根据key计算hash位置
{
...
static void _conf_hashtbl_insert(s_p_hashtbl_t *hashtbl,//插入hash表value值
				 s_p_values_t *value)
{
	int idx;
 ...
 * NOTE - "key" is case insensitive.
 */
static s_p_values_t *_conf_hashtbl_lookup(//查看key是否在hash表中，存在则返回该项，否则返回NULL
...
s_p_hashtbl_t *s_p_hashtbl_create(const s_p_options_t options[])//创建hash表
{
	...
	for (op = options; op->key != NULL; op++) {
		...
		if (op->type == S_P_LINE || op->type == S_P_EXPLINE) {
			...
			expdata->template =
				s_p_hashtbl_create(op->line_options);//递归创建
			expdata->index = (s_p_hashtbl_t*)xmalloc(len);
			expdata->values = NULL;
			value->data = expdata;
		}
		_conf_hashtbl_insert(hashtbl, value);//插入value
	}
 ...
 * pointers */
static void _conf_hashtbl_swap_data(s_p_values_t *data_1,//交换两个项
				    s_p_values_t *data_2)
{
...
static void _conf_file_values_free(s_p_values_t *p)//释放
{
	int i;
	_expline_values_t* v;
