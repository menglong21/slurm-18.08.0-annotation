...
struct slurm_buf {
	uint32_t magic;
	char *head;//char*指针，指向一段空间，即buf内容
	uint32_t size;//buf总大小
	uint32_t processed;//buf已经处理的大小
	bool mmaped;
};

typedef struct slurm_buf * Buf;

#define get_buf_data(__buf)		(__buf->head)//buf数据域
#define get_buf_offset(__buf)		(__buf->processed)//buf已经被处理的部分的大小
#define set_buf_offset(__buf,__val)	(__buf->processed = __val)
#define remaining_buf(__buf)		(__buf->size - __buf->processed)//buf剩余大小
#define size_buf(__buf)			(__buf->size)//buf大小
