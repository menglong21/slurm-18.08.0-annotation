...
/* Gres symbols provided by the plugin由插件提供的gres符号 */
...
/* Gres plugin context, one for each gres type 每个Gres类型对应一个Gres插件上下文*/
typedef struct slurm_gres_context {
	plugin_handle_t	cur_plugin;	/* 插件句柄 */
	char *		gres_name;		/* name (e.g. "gpu") */
	char *		gres_name_colon;	/* name + 冒号 (e.g. "gpu:") */
	...
	slurm_gres_ops_t ops;			/* pointers to plugin symbols *///经典用法，驱动里面经常用，将对象的操作集合单独封装，可以实现多态
	uint32_t	plugin_id;		/* key for searches搜索的关键 */
			     ...
			     slurm_gres_context_t *plugin_context)
{
	...
	/* Find the correct plugin 填充plugin_context*/
	...
	if (plugin_context->plugin_list == NULL) {
		...
		plugrack_read_dir(plugin_context->plugin_list, plugin_dir);//在不同冒号分隔的目录中测试插件		
		xfree(plugin_dir);
	}

	plugin_context->cur_plugin = plugrack_use_by_type(//获取插件句柄
...
extern int gres_plugin_init(void)
{
	...
	gres_plugin_list = slurm_get_gres_plugins();//从slurm.conf配置文件中获取gres信息
	gres_context_cnt = 0;
	...
	one_name = strtok_r(names, ",", &last);//分割字符串
	while (one_name) {
		...
		} else {//此处插件相关的操作堪称经典，值得学习
			xrealloc(gres_context, (sizeof(slurm_gres_context_t) *
				 (gres_context_cnt + 1)));//动态改变gres_context指向的内存大小,相当于不定长数组
			(void) _load_gres_plugin(one_name,//加载插件，解析API，保存插件上下文到gres_context
						 gres_context +
						 gres_context_cnt);//指针移动gres_context_cnt* sizeof(slurm_gres_context_t)，指向下一段内存
			/* Ignore return code.
		...
		one_name = strtok_r(NULL, ",", &last);//连续分割字符串
	}
	xfree(names);
...
/*
 * 加载此节点的配置 (how many resources it has, topology, etc.)
					...
					void *xcpuinfo_abs_to_mac)
{
	...
	if (xcpuinfo_abs_to_mac)
		xcpuinfo_ops.xcpuinfo_abs_to_mac = xcpuinfo_abs_to_mac;//NULL

	rc = gres_plugin_init();//初始化gres插件
	if (gres_context_cnt == 0)
		return SLURM_SUCCESS;

	gres_conf_file = get_extra_conf_path("gres.conf");//从SLURM_CONF宏定义和默认位置获取gres.conf
	...
	tbl = s_p_hashtbl_create(_gres_options);//创建hashtbl
	if (s_p_parse_file(tbl, NULL, gres_conf_file, false) == SLURM_ERROR)//解析文件
		fatal("error opening/reading %s", gres_conf_file);
	FREE_NULL_LIST(gres_conf_list);
	gres_conf_list = list_create(_destroy_gres_slurmd_conf);
	if (s_p_get_array((void ***) &gres_array, &count, "Name", tbl)) {//解析生成gres_conf_list
		for (i = 0; i < count; i++) {
			list_append(gres_conf_list, gres_array[i]);
			gres_array[i] = NULL;
		}
	}
	if (s_p_get_array((void ***) &gres_array, &count, "NodeName", tbl)) {//解析生成gres_conf_list
		for (i = 0; i < count; i++) {
			list_append(gres_conf_list, gres_array[i]);
			gres_array[i] = NULL;
		}
	}
	s_p_hashtbl_destroy(tbl);//销毁hashtbl
	list_for_each(gres_conf_list, _log_gres_slurmd_conf, NULL);//遍历打印节点上的gres信息

	for (i = 0; ((i < gres_context_cnt) && (rc == SLURM_SUCCESS)); i++) {
		_validate_config(&gres_context[i]);
		if (gres_context[i].ops.node_config_load == NULL)
			continue;	/* No plugin */
		rc = (*(gres_context[i].ops.node_config_load))(gres_conf_list);//遍历插件数组，调用每个插件中的node_config_load方法,传入gres_conf_list
	}
	slurm_mutex_unlock(&gres_context_lock);
...
/*
 * 从buffer中解压节点配置 (built/packed by slurmd)，解压出来的信息添加到gres_conf_list
	...
	for (i = 0; i < rec_cnt; i++) {//解包
		if (protocol_version >= SLURM_18_08_PROTOCOL_VERSION) {
	 	...
	 	for (j = 0; j < gres_context_cnt; j++) {
	 		if (gres_context[j].plugin_id != plugin_id)//判断插件id号
				continue;
			if (xstrcmp(gres_context[j].gres_name, tmp_name)) {//判断插件名
				/*
		...
		p = xmalloc(sizeof(gres_slurmd_conf_t));//创建并填充gres_slurmd_conf_t
		p->count = count64;
		p->cpu_cnt = cpu_cnt;
		p->has_file = has_file;
		p->cpus = tmp_cpus;
		...
		_validate_links(p);
		list_append(gres_conf_list, p);//添加到gres_conf_list
	}
