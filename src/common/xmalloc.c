		    ...
		    const char *file, int line, const char *func)
{
	void *new;
	size_t *p;
	size_t total_size = size + 2 * sizeof(size_t);//多分配了2*sizeof(size_t)，

	if (size <= 0)
		return NULL;

	if (clear)
		p = calloc(1, total_size);//动态分配内存并初始化为零
	else
		p = malloc(total_size);//动态分配内存，不初始化
	if (!p) {
		/* out of memory */
		log_oom(file, line, func);
		abort();
	}
	p[0] = XMALLOC_MAGIC;	/* add "secret" magic cookie 上面多分配的两个size_t，第一个存魔数*/
	p[1] = size;		/* store size in buffer 上面多分配的两个size_t，第二个存大小*/
