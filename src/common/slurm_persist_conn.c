	...
	slurm_persist_conn_t *persist_conn, void *arg)
{
	...
	while (!(*persist_conn->shutdown) && !fini) {
		if (!_conn_readable(persist_conn))//poll等待直到文件可读
			break;		/* problem with this socket */
		msg_read = read(persist_conn->fd, &nw_size, sizeof(nw_size));//读取数据长度
		if (msg_read == 0)	/* EOF */
			break;
		...
		while (msg_size > offset) {
			if (!_conn_readable(persist_conn))
				break;		/* problem with this socket */
			msg_read = read(persist_conn->fd, (msg_char + offset),//数据起始地址msg_char,每次读取数据拼接到msg_char+offset后面
					(msg_size - offset));
			if (msg_read <= 0) {
		...
		if (msg_size == offset) {//读完一包数据
			persist_msg_t msg;

			rc = slurm_persist_conn_process_msg(//根据消息类型解包
				persist_conn, &msg,
				msg_char, msg_size,
				&buffer, first);

			if (rc == SLURM_SUCCESS) {
				rc = (persist_conn->callback_proc)(//调用proc_req处理函数
					arg, &msg, &buffer, &uid);
...
{
	persist_service_conn_t *service_conn = arg;//保存参数
	...
	service_conn->thread_id = pthread_self();//保存线程号

	_process_service_connection(service_conn->conn, service_conn->arg);//创建处理线程，处理完返回继续向下执行

	if (service_conn->conn->callback_fini)//结束处理
						...
						int thread_loc, void *arg)
{
	...
	slurm_thread_create(&persist_service_conn[thread_loc]->thread_id,
			    _service_connection, service_conn);//创建_service_connection线程
}
...
extern int slurm_persist_conn_open(slurm_persist_conn_t *persist_conn)
{
	...
	if (slurm_send_node_msg(persist_conn->fd, &req_msg) < 0) {
		...
		Buf buffer = slurm_persist_recv_msg(persist_conn);//从persist_conn接收包
		persist_msg_t msg;
		...
		rc = slurm_persist_msg_unpack(&persist_conn_tmp, &msg, buffer);//解包
		free_buf(buffer);
					  ...
					  Buf *out_buffer, bool first)
{
	...
	rc = slurm_persist_msg_unpack(persist_conn, persist_msg, recv_buffer);//根据消息类型解包
...
extern int slurm_persist_conn_writeable(slurm_persist_conn_t *persist_conn)
{
	...
	while ((*persist_conn->shutdown) == 0) {
		time_left = write_timeout - _tot_wait(&tstart);//poll阻塞时间
		rc = poll(&ufds, 1, time_left);
		if (rc == -1) {
			if ((errno == EINTR) || (errno == EAGAIN))
				continue;
				  ...
				  persist_msg_t *req_msg)
{
	...
	if (persist_conn->flags & PERSIST_FLAG_DBD)//dbd相关的打包
				    ...
				    persist_msg_t *resp_msg, Buf buffer)
{
	...
	if (persist_conn->flags & PERSIST_FLAG_DBD) {
		rc = unpack_slurmdbd_msg((slurmdbd_msg_t *)resp_msg,//根据消息类型解包
					 persist_conn->version,
					 buffer);
	} else {
		slurm_msg_t msg;
