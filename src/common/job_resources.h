 ...
 *
 * NOTES:
 * cpu_array_* contains the same information as "cpus", but in a more compact    cpu_array_*包含与“cpus”相同的信息，但格式更紧凑
 ...
 * build_job_resources_cpu_array()这个函数负责转换
 *
 ...
 *
 * If a job changes size (reliquishes nodes), the node_bitmap will remain如果作业更改大小(释放节点)，节点位图保持不变，
 * unchanged, but cpus, cpus_used, cpus_array_*, and memory_used will be 但cpus, cpus_used, cpus_array_*, and memory_used会更新
 * updated (e.g. cpus and mem_used on that node cleared).
 */
struct job_resources {
	bitstr_t *core_bitmap;		//为所有节点和套接字分配的内核的位图。位图只反映已分配节点上的已分配资源，而不是整个系统资源
	bitstr_t *core_bitmap_used;	//分配给作业步骤的核心的位图
	uint32_t  cpu_array_cnt;	//下面cpu_array_*中元素的计数
	uint16_t *cpu_array_value;	//为作业每个节点分配的cpu数
	uint32_t *cpu_array_reps;	//复制cpu_array_value的连续节点的数量。请参阅下面的注释。	
	uint16_t *cpus;				//作业/步骤的每个节点所需/分配cpu的计数
	uint16_t *cpus_used;		//对于作业，作业步骤使用的每个节点的cpu数 
	uint16_t *cores_per_socket;	//节点上每个套接字的核数，通过build_job_resources()构建,同时确保对core_bitmap的解释是一致的
	uint64_t *memory_allocated;	//为作业或步骤保留的每个节点的MB
	uint64_t *memory_used;		//作业步骤消耗的每个节点内存为MB	
	uint32_t  nhosts;			//分配中的节点数,这应该总是node_bitmap中设置的位数
	bitstr_t *node_bitmap;		//分配给作业的节点的位图。与slurmctld作业记录中的node_bitmap不同，这里的位不会在作业在节点上完成时被清除
	uint32_t  node_req;			//NODE_CR_RESERVED|NODE_CR_ONE_ROW|NODE_CR_AVAILABLE
	char	 *nodes;			//原始作业分配中的节点名称	
	uint32_t  ncpus;			//分配中的处理器数量	
	uint32_t *sock_core_rep_count;//sockets_per_node和cores_per_socket应用于多少个连续节点，通过build_job_resources()构建，并确保core_bitmap的解释是一致的	
	uint16_t *sockets_per_node;	//这个节点上的套接字数量，由build_job_resources()构建并确保core_bitmap的解释是一致的
	uint8_t   whole_node;		//作业分配的全节点(used only by select/cons_res)
};
