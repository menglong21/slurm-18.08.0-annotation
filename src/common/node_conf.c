...
extern int build_all_nodeline_info (bool set_bitmap, int tres_cnt)
{
	...
	count = slurm_conf_nodename_array(&ptr_array);//从conf_hashtbl获得ptr_array和count
	if (count == 0)
	...
	if (set_bitmap) {//创建位图
		...
		while ((config_ptr = (struct config_record *)
			...
			node_name2bitmap(config_ptr->nodes, true,//重要，将节点名称转换为位图
 ...
 *	representation给定一个节点名正则表达式，构建一个位图表示 
