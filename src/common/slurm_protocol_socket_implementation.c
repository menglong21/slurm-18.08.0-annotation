					  ...
					  uint32_t flags, int tmout)
{
	...
	len = slurm_recv_timeout( fd, (char *)&msglen,//接收包长度，内部POLL
	...
	*pbuf = xmalloc_nz(msglen);
	...
	if (slurm_recv_timeout(fd, *pbuf, msglen, 0, tmout) != msglen) {//接收包数据存到pbuf，内部POLL
		xfree(*pbuf);
		*pbuf = NULL;
		return SLURM_ERROR;
	}
			      ...
			      uint32_t flags, int timeout )
{
	...
	gettimeofday(&tstart, NULL);//获得当前时间
	...
	while (recvlen < size) {
		...
		rc = recv(fd, &buffer[recvlen], (size - recvlen), flags);//实际接收信息
		if (rc < 0)  {
			if (errno == EINTR)
				continue;
			else {
