...
Buf create_buf(char *data, uint32_t size)
{
	...
	my_buf = xmalloc_nz(sizeof(struct slurm_buf));//分配一个buf的空间
	my_buf->magic = BUF_MAGIC;
	my_buf->size = size;//数据块大小
	my_buf->processed = 0;//数据块已经处理的大小
	my_buf->head = data;//这块数据是变化的，变化的情况显示在processed中
	my_buf->mmaped = false;
...
int unpackstr_xmalloc_escaped(char **valp, uint32_t *size_valp, Buf buffer)
{
	...
	memcpy(&ns, &buffer->head[buffer->processed], sizeof(ns));
	*size_valp = ntohl(ns);//保存数据包大小，网络字节序到主机字节序的转换
	buffer->processed += sizeof(ns);
