...
static int _load_plugins(void *x, void *arg)//加载插件
{
...
extern int slurm_select_init(bool only_default)
{
	...
	select_type = slurm_get_select_type();//获得select插件类型
	...
	if (only_default) {
		...
		plugin_names = plugin_get_plugins_of_type(plugin_type);//返回与给定类型匹配的插件名称列表
	}
	if (plugin_names && (plugin_cnt = list_count(plugin_names))) {
		...
		select_context = xmalloc(sizeof(plugin_context_t *) *
					 plugin_cnt);
		list_for_each(plugin_names, _load_plugins, &plugin_args);//逐个加载插件
	}
