...
/* 这个slurm文件解析器提供了一种方法，用于为“key = value”形式的键-值对解析文件。
 * 这个解析器可以用于任何类似于slurm的配置文件，而不仅仅是slurm.conf。
 ...
 * 
 * 在解析的文件中，键、等号和值之间允许有任意数量的空白。
 * 解析器自动处理注释、行延续和转义字符。
 * 如果值字符串中需要空白，可以使用双引号来包围整个值。
 ...
 * a value string.
 * 
 * 注释以“”开头，行尾结束。行继续符是行尾的\“字符（只有空格可以跟在\”后面）。
 * 行继续符告诉解析器将下面的行与当前行连接起来。
