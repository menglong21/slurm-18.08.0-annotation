...
extern int slurm_ping(int dest)
{
	...
	rc = _send_message_controller(dest, &request_msg);//向控制器发送信息
...
static int _send_message_controller(int dest, slurm_msg_t *req)
{
	 ...
	 * value of "dest")
	 */
	if ((fd = slurm_open_controller_conn_spec(dest,//1.打开fd,此处用到了working_cluster_rec
						  working_cluster_rec)) < 0) {
		slurm_seterrno_ret(SLURMCTLD_COMMUNICATIONS_CONNECTION_ERROR);
	}

	if (slurm_send_node_msg(fd, req) < 0) {//2.发送信息
		close(fd);
		slurm_seterrno_ret(SLURMCTLD_COMMUNICATIONS_SEND_ERROR);
	}
	slurm_msg_t_init(&resp_msg);

	if ((rc = slurm_receive_msg(fd, &resp_msg, 0)) != 0) {//3.接收返回信息
		slurm_free_msg_members(&resp_msg);
		close(fd);
		return SLURMCTLD_COMMUNICATIONS_RECEIVE_ERROR;
	}

	if (close(fd) != SLURM_SUCCESS)//4.关闭fd
