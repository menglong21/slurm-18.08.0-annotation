	...
	*node_info_msg_pptr = NULL;
	...
	while ((cluster = (slurmdb_cluster_rec_t *) list_next(iter))) {
		...
		slurm_thread_create(&load_thread[pthread_count],
				    _load_node_thread, load_args);//对每个集群创建线程
		pthread_count++;
	}
			   ...
			   uint16_t show_flags)
{
	...
	if ((show_flags & SHOW_FEDERATION) && ptr) { /* "ptr" check for CLANG */
		...
		rc = _load_fed_nodes(&req_msg, resp, show_flags, cluster_name,//获取联合集群内node信息，对联合内每个集群创建一个线程，线程内调用_load_cluster_nodes
				     fed);
	} else {
		rc = _load_cluster_nodes(&req_msg, resp, working_cluster_rec,//获取本集群或者多集群的node信息
					 show_flags);
	}
