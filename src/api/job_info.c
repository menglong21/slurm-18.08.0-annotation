...
//这组函数加载/释放节点信息，以便我们可以根据每个节点上的线程计数将作业的核心位图映射到它的CPU ID。
static void _load_node_info(void)
{
	slurm_mutex_lock(&job_node_info_lock);
	if (!job_node_ptr)//如果作业节点信息为空，则加载作业节点信息
		(void) slurm_load_node((time_t) NULL, &job_node_ptr, 0);
	slurm_mutex_unlock(&job_node_info_lock);
}

static uint32_t _threads_per_core(char *host)//这个函数依赖于上面_load_node_info函数获取的作业节点信息
{
	...
	for (i = 0; i < job_node_ptr->record_count; i++) {
		if (job_node_ptr->node_array[i].name &&
			...
			threads = job_node_ptr->node_array[i].threads;//每个核心的线程数,一般是1
			break;
		}
	}
	slurm_mutex_unlock(&job_node_info_lock);
...
slurm_print_job_info ( FILE* out, job_info_t * job_ptr, int one_liner )
{
	...
	if ((print_this = slurm_sprint_job_info(job_ptr, one_liner))) {
		fprintf(out, "%s", print_this);//向文件out里面写入print_this信息
		xfree(print_this);
	}
	_free_node_info();
}
...
slurm_sprint_job_info ( job_info_t * job_ptr, int one_liner )
{
	...
	job_resources_t *job_resrcs = job_ptr->job_resrcs;//此处获得job_resrcs
	char *out = NULL;
	time_t run_time;
	...
	if (job_resrcs && job_resrcs->core_bitmap &&//三个条件作业资源结构体存在且其中位图存在且不为空
		...
		for (rel_node_inx=0; rel_node_inx < job_resrcs->nhosts;
			...
			cpu_bitmap = bit_alloc(bit_reps * threads);//分配位串
			for (j = 0; j < bit_reps; j++) {
				if (bit_test(job_resrcs->core_bitmap, bit_inx)){
					for (k = 0; k < threads; k++)
						bit_set(cpu_bitmap,	//设置位串	
							(j * threads) + k);
				}
				bit_inx++;
			}
			bit_fmt(tmp1, sizeof(tmp1), cpu_bitmap);//格式化位串，使用gdb追踪位串和tmp1和tmp2的变化
			 ...
			 */
			if (xstrcmp(tmp1, tmp2) ||//比较tmp1和tmp2
				...
				if (hostlist_count(hl_last)) {
					...
					xstrfmtcat(out,
						   "  Nodes=%s CPU_IDs=%s "
						   "Mem=%"PRIu64" GRES_IDX=%s",//使用PRIu64打印uint64
						   last_hosts, tmp2,
						   last_mem_alloc_ptr ?
						   last_mem_alloc : 0,
						   gres_last);
		   ...
		   slurmdb_cluster_rec_t *cluster)
{
	...
	*job_info_msg_pptr = NULL;
	//跟指定cluster的slurmctld通讯，获取信息resp_msg
	if (slurm_send_recv_controller_msg(req_msg, &resp_msg, cluster) < 0)
		return SLURM_ERROR;

	switch (resp_msg.msg_type) {
	case RESPONSE_JOB_INFO:
		*job_info_msg_pptr = (job_info_msg_t *)resp_msg.data;//获得有效作业信息
		resp_msg.data = NULL;
		break;
...
static void *_load_job_thread(void *args)
{
	...
	int rc;
	//每个线程实际还是调用_load_cluster_jobs，向cluster的slurmctld发送rpc请求
...
//对联合集群内个每个集群建立一个线程，线程内调用_load_cluster_jobs，向cluster的slurmctld发送rpc请求
	...
	*job_info_msg_pptr = NULL;
	...
	while ((cluster = (slurmdb_cluster_rec_t *) list_next(iter))) {
		...
		slurm_thread_create(&load_thread[pthread_count],
				    _load_job_thread, load_args);//创建线程，跟slurmctld通信获取作业信息
		pthread_count++;
 ...
 */
 //发出RPC以获取自update_time以来更改的所有作业配置信息，REQUEST_JOB_INFO
extern int
		 ...
		 uint16_t show_flags)
{
	...
	slurm_msg_t_init(&req_msg);//初始化req_msg
 ...
 * NOTE: free the response using slurm_free_job_info_msg
 */
//打包消息REQUEST_JOB_INFO_SINGLE，向本集群或者多集群或者联合集群的slurmctld发送请求
extern int
		...
		uint16_t show_flags)
{
	 ...
	 * information for that cluster */
	if (working_cluster_rec || !ptr || (show_flags & SHOW_LOCAL)) {
		rc = _load_cluster_jobs(&req_msg, job_info_msg_pptr,//跟指定cluster的slurmctld通讯，获取信息resp_msg
					working_cluster_rec);
	} else {
		fed = (slurmdb_federation_rec_t *) ptr;
		rc = _load_fed_jobs(&req_msg, job_info_msg_pptr, show_flags,//为fed里面每个集群生成一个pthread以收集作业信息
				    cluster_name, fed);
	}
