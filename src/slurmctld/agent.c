 ...
 *  used by the watchdog thread as well as the communication threads.
 下面的函数允许slurm作为一个分离的线程启动并行任务，并让下面的函数确保工作正常进行。
 例如，当要更改作业的时间限制时，slurmctld需要在分配作业的每个节点上通知slurmd。
 我们不想挂起slurmctld的主要功能(作业更新RPC)来执行这项工作，所以它只启动一个代理来执行这项工作。
 agent将传递执行工作所需的所有详细信息，因此可以将代理作为pthread、process甚至其他计算机上的守护进程来执行。
 主代理线程为每个节点创建一个单独的线程，以便与AGENT_THREAD_COUNT通信。
 一个特殊的看门狗线程将SIGLARM发送给任何已经活动(处于DSH_ACTIVE状态)超过MessageTimeout秒的线程。
 代理根据需要通过函数调用或RPC对slurmctld作出响应。
 例如，通知slurmctld某些节点没有响应。
 每个线程的所有状态都保存在thd_t结构中，监视线程和通信线程都使用thd_t结构。
\*****************************************************************************/
...
typedef struct thd {
	...
	state_t state;			/* 线程 state */
	time_t start_time;		/* start time */
	time_t end_time;		/* end time or delta time
					 * upon termination */
	slurm_addr_t *addr;		/* 要发送到的特定addr
					 * will not do nodelist if set */
	char *nodelist;			/* 要发送到的节点列表 */
	List ret_list;
} thd_t;
...
typedef struct agent_info {
	...
	uint32_t thread_count;		/* 线程记录数 */
	uint32_t threads_active;	/* 当前活动线程数 */
	uint16_t retry;			/* if set, keep trying */
	thd_t *thread_struct;		/* 线程结构体 */
	bool get_reply;			/* flag if reply expected */
	slurm_msg_type_t msg_type;	/* 将发出的RPC */
	void **msg_args_pptr;		/* 要使用的RPC数据 */
 ...
 */
//负责在一组节点（slurmd）之间并行传输通用RPC。
//如果不需要立即执行，则使用agent_queue_request()。
void *agent(void *args)
{
	...
	while (1) {
		if (slurmctld_config.shutdown_time ||
			...
			//agent_thread_cnt超过限制，线程被条件变量agent_cnt_cond阻塞
			//条件变量必须和互斥锁配合使用，调用pthread_cond_wait()前必须由本线程加锁,
			//pthread_cond_wait会解锁并阻塞在等待队列中休眠，直到被唤醒
			//当等待条件成立,线程被唤醒并已经锁定
	...
	//确认agent_arg_ptr->node_count
	if (_valid_agent_arg(agent_arg_ptr))
		goto cleanup;

#if defined HAVE_NATIVE_CRAY//没用
	...
	//初始化agent数据结构，内部初始化好了thread_count
	...
	for (i = 0; i < agent_info_ptr->thread_count; i++) {
		...
		//创建_thread_per_group_rpc线程，将task_specific_ptr作为参数传入
	...
	slurm_cond_broadcast(&agent_cnt_cond);//本agent结束触发agent_cnt_cond，启动其他agent
	slurm_mutex_unlock(&agent_cnt_mutex);
 ...
 */
 //线程为一组节点发出RPC，将消息发送到其中一个节点，并在必要时将其转发给其他节点。
static void *_thread_per_group_rpc(void *args)
{
	...
	if (task_ptr->get_reply) {
		if (thread_ptr->addr) {
			...
			if (!(ret_list = slurm_send_recv_msgs(//向指定的节点列表发送消息
				     thread_ptr->nodelist,
				     &msg, 0, true))) {
				error("%s: no ret_list given", __func__);
				goto cleanup;
	...
	while ((ret_data_info = list_next(itr)) != NULL) {
		...
		switch (rc) {
		case SLURM_SUCCESS://agent将rpc发给node(slurmd)
...
static void *_agent_init(void *arg)
{
	...
	while (true) {
		...
		//1.shutdown_time
		//2.pending_mail
		//3.pending_wait_time值被改变，agent_trigger
		while (!slurmctld_config.shutdown_time &&
		       !pending_mail && (pending_wait_time == NO_VAL16)) {
			ts.tv_sec  = time(NULL) + 2;
			//阻塞，等待penging_cond或者超时，agent_trigger触发
...
extern void agent_trigger(int min_wait, bool mail_too)
{
	...
	slurm_cond_broadcast(&pending_cond);//触发pending_cond
	slurm_mutex_unlock(&pending_mutex);
}
...
static void _agent_retry(int min_wait, bool mail_too)
{
	...
	lock_slurmctld(job_write_lock);
	slurm_mutex_lock(&retry_mutex);
	//如果retry_list存在取前五个消息，打印消息类型
	//问题：retry_list什么时候产生的？
	//解答：agent_queue_request会将不用立即执行的请求放到retry_list，延后触发执行调度
	if (retry_list) {
	...
	slurm_mutex_unlock(&agent_cnt_mutex);
	//找新的请求，测试是否可延迟，如果需要中断请求则释放该请求占用的资源，删除该请求
	if (retry_list) {
...
/*	queued_req_ptr->last_attempt  = 0; Implicit */
	...
	if (retry_list == NULL)
		retry_list = list_create(_list_delete_retry);
	list_append(retry_list, (void *)queued_req_ptr);//将queued_req_ptr添加到retry_list，重要
	slurm_mutex_unlock(&retry_mutex);

	/* now process the request in a separate pthread
	 * (if we can create another pthread to do so) */
	agent_trigger(999, false);//触发阻塞的agent
}
