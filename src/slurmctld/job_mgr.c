...
/* Global variables */
List   job_list = NULL;		/* job_record list 运行中作业记录链表，重要*/
 ...
 */
 //创建一个包含job_details的空job_record，并添加到全局链表job_list
static struct job_record *_create_job_record(uint32_t num_jobs)
{
	struct job_record  *job_ptr;
	...
	(void) list_append(job_list, job_ptr);//重要一步
 ...
 */
 //给定分区名，释放其作业的资源并杀死它们。与此分区关联的所有作业的分区指针都将被清除。
 ...
 */
 //给定前端节点名，释放其作业的资源并杀死它们。front end node节点什么意思？
 ...
 */
 //给定一个节点名，从该节点释放正在运行或正在完成的作业，或者杀死它们
...
static int _job_fail(struct job_record *job_ptr, uint32_t job_state)
{
	...
	if (IS_JOB_CONFIGURING(job_ptr) || IS_JOB_RUNNING(job_ptr) ||
		...
		xfree(job_ptr->state_desc);
		job_completion_logger(job_ptr, false);//记录作业信息
		if (job_ptr->node_bitmap) {
			build_cg_bitmap(job_ptr);
...
/*
 * Signal a job based upon job pointer.根据作业指针发出作业信号。
...
/*
 * job_str_signal - signal the specified job向指定的作业发出信号
...
/*
 * job_complete - note the normal termination the specified job注意指定作业的正常终止
 * IN job_id - id of the job which completed
...
//FIXME: TRES counts not necessarily established for GRES at this point
	...
	//解析作业依赖项字符串，并使用它建立依赖项的“依赖_规范”列表。
			     ...
			     bitstr_t ** exc_bitmap)
{
	...
	*job_rec_ptr = job_ptr;//*job_rec_ptr将把填充好的作业结构体带回去
	...
	_add_job_hash(job_ptr);//添加作业哈希，重要，方便以后查找作业
			  ...
			  uint16_t protocol_version)
{
	...
	*buffer_size = 0;
	...
	itr = list_iterator_create(job_list);//job_list非常重要，是所有运行的作业的链表
...
static inline bool _purge_complete_pack_job(struct job_record *pack_leader)
{
	...
	if (i) {
		...
		last_job_update = time(NULL);
		slurm_cond_signal(&purge_thread_cond);//触发purge_thread_cond
	}
	return true;
}
...
{
	int count = 0;
	count = list_delete_all(job_list, &list_find_job_id, (void *)&job_id);//注意这个函数的写法，值得学习
	if (count) {
		last_job_update = time(NULL);
		slurm_cond_signal(&purge_thread_cond);//触发purge_thread_cond
	}
 ...
 *	job_list - pointer to global job list
 */
 //为现有作业重建位图。这应该在重新生成节点信息之后，但在使用任何作业条目之前调用。
void reset_job_bitmaps(void)
{
	...
	while ((job_ptr = (struct job_record *) list_next(job_iterator))) {
		...
		if (!job_fail && !IS_JOB_FINISHED(job_ptr) &&
		    ...
		    valid_job_resources(job_ptr->job_resrcs,//确认作业资源
		...
		if (_reset_detail_bitmaps(job_ptr))//重置细节位图
			job_fail = true;
 ...
 *	records.如果没有清理作业记录和/或节点记录，则验证应该在节点上的任何作业实际上正在运行。
 ...
 *	remove it the list (of directories to be deleted) */
 //所有挂起的批处理作业都必须有一个batch_dir条目，否则我们将其标记为FAILED，不进行调度
static void _validate_job_files(List batch_dirs)
{
	struct job_record *job_ptr;
 ...
 * subsequent jobs appear in a separate accounting record. */
 //完成所有步骤后，完成批处理作业重新排队逻辑，以便后续作业显示在单独的会计记录中。
...
//将一个完成的作业信息保存到存储中，这个函数一般在一个作业的生命周期的最后执行
extern void job_completion_logger(struct job_record *job_ptr, bool requeue)
{
	...
	jobacct_storage_g_job_complete(acct_db_conn, job_ptr);//实际存储过程
}
...
extern void build_cg_bitmap(struct job_record *job_ptr)
{
	FREE_NULL_BITMAP(job_ptr->node_bitmap_cg);
	if (job_ptr->node_bitmap) {
		job_ptr->node_bitmap_cg = bit_copy(job_ptr->node_bitmap);//返回提供的位图的副本
		if (bit_set_count(job_ptr->node_bitmap_cg) == 0)
			job_ptr->job_state &= (~JOB_COMPLETING);
	} else {
		error("build_cg_bitmap: node_bitmap is NULL");
		job_ptr->node_bitmap_cg = bit_alloc(node_record_count);//node_record_count全局变量，节点记录计数
		job_ptr->job_state &= (~JOB_COMPLETING);
	}
}
...
_kill_dependent(struct job_record *job_ptr)
{
	...
	job_ptr->start_time = now;
	job_ptr->end_time = now;
	job_completion_logger(job_ptr, false);//记录作业
	last_job_update = now;
	srun_allocate_abort(job_ptr);
}
