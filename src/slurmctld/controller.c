...
int main(int argc, char **argv)
{
	...
	//读取slurm配置文件，初始化slurm_ctl_conf_t结构体（slurm.h）
	...
	slurm_conf_reinit(slurm_conf_filename);//本文件_parse_commandline内被初始化
	 ...
	 * fail.
	 */
	 //验证conf中的clustername与spool dir中的值匹配
	 //如果不一致，退出以保护状态文件不受损坏。
	 //spool/clustername
	...
	if (!xstrcasecmp(slurmctld_conf.accounting_storage_type,
		...
		init_job_conf();//如果通过slurmdbd记账，初始化作业链表
	}
	...
	//设置slurm strigger回调函数，主要是设置一些静态全局变量
	...
	//初始化作业凭证，配置文件中指定JobCredentialPrivateKey
	...
	//HA,High Available高可用，找到本机在所有controller里面的索引号
	...
	while (1) {
		...
		/* 以主模式或备份模式启动 */
		if (!slurmctld_primary) {//备份控制器
			slurm_sched_fini();	/* make sure shutdown */
			_run_primary_prog(false);//false以备份控制器运行,true以主控制器运行
			run_backup(&callbacks);//这是备份控制器，它应该以备用模式运行，假设主控制器停止响应时进行控制
		...
		} else if (test_config || slurmctld_primary) {//主控制器
			if (!test_config) {
				(void) _shutdown_backup_controller();//让备控制器放弃控制
				trigger_primary_ctld_res_ctrl();
				ctld_assoc_mgr_init(&callbacks);//确保assoc_mgr已启动并以当前状态运行
			}
			//初始化acct_storage插件
			if (slurm_acct_storage_init(NULL) != SLURM_SUCCESS) {
				if (test_config) {
			...
			if (recover == 0) {
				slurmctld_init_db = 1;
				//冷启动，从spool里读取node_state
				_accounting_mark_all_nodes_down("cold-start");
			}
		}

		if (!acct_db_conn) {//仅一次
			acct_db_conn = acct_storage_g_get_connection(//打开一个跟slurmdbd的socket连接
			 ...
			 */
			 //我们只在第一次调用它的时候发送一个变量，
			 //因为我们在函数内部设置静态变量，发送NULL只会使用之前的设置。
			 //初始化priority_type，tres,qos,user,assoc,wckey等
		...
		_run_primary_prog(true);//true按主控制器运行
		control_time = time(NULL);
		heartbeat_start();//启动心跳线程
		//设置trigger_pri_ctld_res_op
		if ((slurmctld_config.resume_backup == false) &&
		    (slurmctld_primary == 1)) {
			trigger_primary_ctld_res_op();
		}
		//在端口slurmctld_port向slurmdbd注册slurmctld
...
static void *_slurmctld_signal_hand(void *no_data)
{
	...
	for (i = 0; sig_array[i]; i++)
		_default_sigaction(sig_array[i]);//设置sig_array所有信号
	while (1) {
		xsignal_sigset_create(sig_array, &set);//将信号添加进信号集set
		rc = sigwait(&set, &sig);//等待信号
		if (rc == EINTR)
			continue;
		switch (sig) {
		case SIGINT:	/* kill -2  or <CTRL-C> *///中断信号
		case SIGTERM:	/* kill -15 *///终止信号
			info("Terminate signal (SIGINT or SIGTERM) received");
			slurmctld_config.shutdown_time = time(NULL);
			slurmctld_shutdown();
			return NULL;	/* Normal termination */
			break;
		case SIGHUP:	/* kill -1 *///挂起信号
			info("Reconfigure signal (SIGHUP) received");
			_reconfigure_slurm();
			break;
		case SIGABRT:	/* abort *///异常终止信号
		...
		case SIGUSR2://用户信号2
			...
			update_logging();//基于配置参数重置slurmctld日志记录
			break;
		default:
...
static void *_slurmctld_rpc_mgr(void *no_data)
{
	...
	nports = slurmctld_conf.slurmctld_port_count;//slurmctld通讯端口数量，1个到10个
	if (nports == 0) {
	...
	while (_wait_for_server_thread()) {//新来一个rpc,进入循环
		 ...
		 *poll本质上和select没有区别，没有连接数限制，缺点：
		 *1、大量的fd的数组被整体复制于用户态和内核地址空间之间
		 *2、poll还有一个特点是“水平触发”，如果报告了fd后，没有被处理，那么下次poll时会再次报告该fd
		 *epoll优点：
		 *1、没有最大并发连接的限制，能打开的FD的上限远大于1024（1G的内存上能监听约10万个端口）
		 *2、效率提升，不是轮询而是设置回调的方式，不会随着FD数目的增加效率下降
		 *3、内存拷贝，利用mmap()文件映射内存加速与内核空间的消息传递；即epoll使用mmap减少复制开销
		 */
		if (select(max_fd+1, &rfds, NULL, NULL, NULL) == -1) {//阻塞，返回正数说明有连接
			if (errno != EINTR)
		...
		/* find one to process */
		for (i=0; i<nports; i++) {
			if (FD_ISSET(sockfd[(fd_next+i) % nports], &rfds)) {//检查一下
				i = (fd_next + i) % nports;
				break;
			}
		}
...
static bool _wait_for_server_thread(void)
{
	...
	while (1) {
		...
		if (slurmctld_config.server_thread_count < max_server_threads) {
			...
			//阻塞等待slurmctld_config.thread_count_cond
...
extern void server_thread_decr(void)
{
	...
	slurm_cond_broadcast(&slurmctld_config.thread_count_cond);//通知_wait_for_server_thread()
	slurm_mutex_unlock(&slurmctld_config.thread_count_lock);
}
 ...
 *	ping other nodes
 */
 //进程slurmctld后台活动清除失效的作业记录、保存状态、调度作业和ping其他节点
static void *_slurmctld_background(void *no_data)
{
	...
	last_node_acct = now;
	//用来设置线程是否可以被其他线程调用pthread_cancel函数取消/终止。
	(void) pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);//可取消
	(void) pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);//线程接收到取消操作后，立即取消。
	debug3("_slurmctld_background pid = %u", getpid());

	while (1) {//果然,这里是一个死循环，轮询模式
		for (i = 0; ((i < 10) && (slurmctld_config.shutdown_time == 0));
		     i++) {
			usleep(100000);//0.1秒，十次
		}
		...
		if (slurmctld_config.shutdown_time) {//_slurm_rpc_shutdown_controller设置shutdown_time
			...
			if (!report_locks_set()) {
				...
				save_all_state();//保存整个slurmctld状态，以便稍后恢复
			} else {
		...
		if (job_limit != NO_VAL) {
			...
			if (schedule(job_limit)) //尝试调度所有挂起的作业
		...
		if (difftime(now, last_checkpoint_time) >=
			...
			save_all_state();//保存状态，触发slurmctld_state_save线程执行保存的工作
		}
 ...
 */
 //告诉备份控制器放弃控制，主控制器已经恢复运行。消息并发地发送给所有控制器。
static int _shutdown_backup_controller(void)
{
	int i, *arg;
...
extern void set_slurmctld_state_loc(void)
{
	...
	else if (access(path, R_OK|W_OK|X_OK) < 0)//检查权限
...
static void *_purge_files_thread(void *no_data)
{
	...
	while (!slurmctld_config.shutdown_time) {
		//阻塞，等待触发条件变量purge_thread_cond
		//1.controller终止，main
		//2.purge_old_job，_slurmctld_background
		//3._purge_complete_pack_job
		//4.purge_job_record
