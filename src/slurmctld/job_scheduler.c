...
extern int schedule(uint32_t job_limit)
{
	...
	if (delta_t >= sched_min_interval) {
		...
		sched_pend_thread = 1;
		slurm_thread_create_detached(NULL, _sched_agent, NULL);//启动_sched_agent，延后调度
		slurm_mutex_unlock(&sched_mutex);
	} else {
...
extern void launch_job(struct job_record *job_ptr)
{
	...
	launch_msg_ptr = _build_launch_job_msg(launch_job_ptr,protocol_version);//创建job_msg
	...
	agent_queue_request(agent_arg_ptr);//通过代理发送RPC
}
 ...
 */
 //确定挂起的作业是否仅使用指定的节点(在job_desc_msg->req_nodes中)运行，
 //构建响应消息并在成功时返回SLURM_SUCCESS。否则返回错误代码。调用者必须释放响应消息
...
next_part:
	...
	if (rc == SLURM_SUCCESS) {
		...
		//orig_start基于回填调度器数据，并考虑所有更高优先级的作业。
		//下面的逻辑只考虑当前正在运行的作业，因此预期的启动时间几乎肯定会更早，而且不那么准确，但这种算法要快得多。
