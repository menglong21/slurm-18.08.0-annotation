...
//重试一个不完整的RPC代理请求每RPC_RETRY_INTERVAL秒
...
typedef struct slurmctld_config {
	...
	bool	resume_backup;//恢复备份
	bool    scheduling_disabled;
	int	server_thread_count;//服务器线程数
	time_t	shutdown_time;//shutdown time
...
/*****************************************************************************\
 *  RESERVATION parameters and data structures预约相关参数和数据结构
\*****************************************************************************/
...
/*****************************************************************************\
 *  JOB parameters and data structures作业相关参数和数据结构
\*****************************************************************************/
...
struct job_record {
	...
	char *clusters;			/* clusters job is submitted to with -M
					   option */
	char *comment;			/* arbitrary comment 随机注释信息*/
	...
	//gres相关
	List gres_list;			/* generic resource allocation detail通用资源分配细节 */
	char *gres_alloc;		/* Allocated GRES added over all nodes在所有要传递给slurmdbd的节点上添加的已分配GRES
					 * to be passed to slurmdbd */
	uint32_t gres_detail_cnt;	/* Count of gres_detail_str records, gres_detail_str记录的计数，每个分配的节点一个
					 * one per allocated node */
	char **gres_detail_str;		/* Details of GRES index alloc per node 每个节点的gres索引分配详细信息*/
	char *gres_req;			/* Requested GRES added over all nodes在要传递给slurmdbd的所有节点上添加请求的gres
					 * to be passed to slurmdbd */
	char *gres_used;		/* Actual GRES use added over all nodes在要传递给slurmdbd的所有节点上添加的实际gres使用
					 * to be passed to slurmdbd */
	uint32_t group_id;		/* group submitted under 提交的组*/
	//作业id
...
struct 	step_record {
	...
	List gres_list;			/* generic resource allocation detail 通用资源分配细节*/
