...
void slurmctld_req(slurm_msg_t *msg, connection_arg_t *arg)
{
	...
	case REQUEST_JOB_INFO:
		_slurm_rpc_dump_jobs(msg);//scontrol show jobs
		break;
	case REQUEST_JOB_USER_INFO:
		_slurm_rpc_dump_jobs_user(msg);
		break;
	case REQUEST_JOB_INFO_SINGLE:
		_slurm_rpc_dump_job_single(msg);//scontrol show jobs <jobid>
		break;
	...
	case MESSAGE_NODE_REGISTRATION_STATUS:
		_slurm_rpc_node_registration(msg, 0);//处理节点注册
		break;
...
static int _pack_job_cancel(void *x, void *arg)
{
	...
	job_ptr->exit_code	= 1;
	job_completion_logger(job_ptr, false);//记录作业数据
					     ...
					     bool running_composite)
{
	...
	/* Mark job allocation complete 标志作业分配完成*/
...
static void _slurm_rpc_shutdown_controller(slurm_msg_t * msg)
{
	...
	if (error_code)
		...
		if (slurmctld_config.thread_id_sig)	/* signal clean-up */
			...
			slurmctld_config.shutdown_time = now;//此时设置slurmctld_config.shutdown_time
