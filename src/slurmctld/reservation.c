...
static void *_fork_script(void *x)
{
	resv_thread_args_t *args = (resv_thread_args_t *) x;//线程参数
	char *argv[3], *envp[1];
	int status, wait_rc;
	pid_t cpid;
	uint16_t tm;
 ...
 */
 //根据需要为node_state设置或清除NODE_STATE_MAINT
extern int set_node_maint_mode(bool reset_all)
{
	...
	while ((resv_ptr = (slurmctld_resv_t *) list_next(iter))) {
		...
		if ((resv_ptr->start_time <= now) && !resv_ptr->run_prolog) {
			...
			_run_script(slurmctld_conf.resv_prolog, resv_ptr);//运行前处理作业
		}
		if ((resv_ptr->end_time <= now) && !resv_ptr->run_epilog) {
			resv_ptr->run_epilog = true;
			_run_script(slurmctld_conf.resv_epilog, resv_ptr);//运行后处理作业
		}
	}
	list_iterator_destroy(iter);
