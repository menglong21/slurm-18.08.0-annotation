...
//如果slurmctld崩溃，其恢复的节点状态可能与实际的硬件状态不同(例如，ResumeProgram未能完成)。
//为了解决这个问题，当一个应该为正在运行的作业启动的节点没有响应时，他们会再次尝试运行ResumeProgram。
static void _re_wake(void)
{
	struct node_record *node_ptr;
...
static void *_init_power_save(void *arg)
{
	...
	while (slurmctld_config.shutdown_time == 0) {
		...
		_reap_procs();//获取先前fork的子进程以修改节点状态。

		if ((last_config != slurmctld_conf.last_update) &&
		    (_init_power_config())) {//初始化电源管理配置
