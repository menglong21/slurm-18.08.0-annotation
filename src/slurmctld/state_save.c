	...
	while (1) {
		...
		while (1) {
			...
			if (save_count &&
				...
				ts.tv_sec = now + 1;
				//阻塞，等待_slurmctld_background触发或超时
				slurm_cond_timedwait(&state_save_cond,
					  	     &state_save_lock, &ts);
			} else {		/* wait for more work */
				slurm_cond_wait(&state_save_cond,//阻塞，等待_slurmctld_background触发
					  	&state_save_lock);
			}
		}
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)dump_all_front_end_state();//将所有前端节点的状态保存到文件中
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)dump_all_job_state();//保存所有作业信息到文件中
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)dump_all_node_state();//将所有节点的状态保存到文件中
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)dump_all_part_state();//将所有分区的状态保存到文件中
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)dump_all_resv_state();//将所有reservation的状态保存到文件中
		...
		slurm_mutex_unlock(&state_save_lock);
		if (run_save)
			(void)trigger_state_save();//将所有trigger的状态保存到文件中	
	}
}
