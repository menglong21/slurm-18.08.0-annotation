 ...
 */
//构建节点位图来定义哪些节点处于1)分区2)配置记录3)运行状态4)空闲状态。
//还设置每个分区的TEATEN_NODES和TALTER_CPU的值。
static int _build_bitmaps(void)
{
 ...
 *		    slurm.conf contents不使用保存的状态信息，从slurm.conf内容重建所有内容。
 *              1 = recover saved job and trigger state,恢复保存的作业和触发器状态、节点中断/耗尽/失败状态和原因信息
 *                  node DOWN/DRAIN/FAIL state and reason information
 *              2 = recover all saved state恢复所有保存的状态
