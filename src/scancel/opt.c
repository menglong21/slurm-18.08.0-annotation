...
static void _opt_args(int argc, char **argv)
{
	...
	//https://www.cnblogs.com/chenliyang/p/6633739.html  getopt_long解析命令行长参数
	/*char*optstring = “ab:c::”;
...
_xlate_job_step_ids(char **rest)
{
	...
	for (i = 0; rest[i]; i++) {
		id_args[i] = xstrdup(rest[i]);//将rest里面各项导到id_args
		if ((i + 4) >= id_args_size) {//id_args_size自动扩展
			id_args_size *= 2;
			id_args = xrealloc(id_args, sizeof(char *) *
					   id_args_size);//id_args自动扩展
		}
	}

	for (i = 0; id_args[i] && (buf_offset < buf_size); i++) {
		job_id = strtol(id_args[i], &next_str, 10);//将数字字符转换为10进制数字
		if (job_id <= 0) {
			error ("Invalid job id %s", id_args[i]);
			exit (1);
		}
		opt.job_id[buf_offset] = job_id;//将上面获得的作业号job_id存入opt.job_id
