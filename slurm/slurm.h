...
#ifndef __SUGON_MULTICLUSTER
#define __SUGON_MULTICLUSTER
#endif
 ...
 */
#define SLURM_VERSION_NUMBER 0x120800
 ...
 */
typedef struct job_descriptor {	/* 用于提交、分配和更新请求 */
	char *account;		/* 记入指定帐户 */
	char *acctg_freq;	/* 记账的轮询间隔 (seconds) */
	char *admin_comment;	/* 管理员的任意评论 (update only) */
	char *alloc_node;	/* 发出资源分配请求的节点
				 * NOTE: Normally set by slurm_submit* or
				 * slurm_allocate* function */
	uint16_t alloc_resp_port; /* 发送分配确认到的端口 */
...
typedef struct job_info {
	...
	job_resources_t *job_resrcs; /* opaque data type, job resources不透明数据类型、作业资源 */
	...
	char *nodes;		/* list of nodes allocated to job分配给作业的节点链表 */
	uint32_t nice;	  	/* requested priority change */
	int32_t *node_inx;	/* list index pairs into node_table for *nodes:为上面*nodes列出对应到node_table的索引
				 * start_range_1, end_range_1,
				 * start_range_2, .., -1  */
	uint16_t ntasks_per_core;/* number of tasks to invoke on each core */
	uint16_t ntasks_per_node;/* number of tasks to invoke on each node */
...
typedef struct node_info {
	char *arch;		/* computer architecture 计算机体系结构*/
	uint16_t boards;        /* total number of boards per node  每个节点的板总数*/
	time_t boot_time;	/* time of node boot 节点启动时间*/
	char *cluster_name;	/* Cluster name ONLY set in federation 唯一设置在联合以内的集群名称*/
	uint16_t cores;         /* number of cores per socket   每个socket的核心数量    */
	uint16_t core_spec_cnt; /* number of specialized cores on node 节点上的专有核心数*/
	uint32_t cpu_bind;	/* Default task binding 默认任务绑定*/
	uint32_t cpu_load;	/* CPU load * 100 CPU负载*100*/
	uint64_t free_mem;	/* free memory in MiB 空闲内存M*/
	uint16_t cpus;		/* configured count of cpus running on在节点上运行的cpu的配置计数
				 * the node */
	char *cpu_spec_list;	/* node's specialized cpus 节点的专用cpu数*/
	acct_gather_energy_t *energy;	 /* energy data 能源数据*/
	ext_sensors_data_t *ext_sensors; /* external sensor data 外部传感器数据*/
	power_mgmt_data_t *power;        /* power management data 电源管理数据*/
	char *features;		/* list of a node's available features 节点的可用功能列表*/
	char *features_act;	/* list of a node's current active features,节点当前活动功能的列表，如果为NULL，则与“features”相同
				 * Same as "features" if NULL */
	char *gres;		/* list of a node's generic resources 节点通用资源列表*/
	char *gres_drain;	/* list of drained GRES 已耗尽的GRES列表*/
	char *gres_used;	/* list of GRES in current use 当前使用的GRES列表*/
	char *mcs_label;	/* mcs label if mcs plugin in use mcs标签如果mcs插件在使用*/
	uint64_t mem_spec_limit; /* MB memory limit for specialization 内存限制*/
	char *name;		/* node name to slurm 节点名称*/
	uint32_t next_state;	/* state after reboot (enum node_states) 重启后状态*/
	char *node_addr;	/* communication name (optional) 地址*/
	char *node_hostname;	/* node's hostname (optional) 主机名*/
	uint32_t node_state;	/* see enum node_states节点状态 */
	char *os;		/* operating system currently running当前运行的操作系统 */
	uint32_t owner;		/* User allowed to use this node or NO_VAL 允许用户使用此节点或no_val*/
	char *partitions;	/* Comma separated list of partitions containing包含此节点的以逗号分隔的分区列表，
				 * this node, NOT supplied by slurmctld, but该列表不是由slurmctld提供的，而是由scontrol填充的。
				 * populated by scontrol */
	uint16_t port;		/* TCP port number of the slurmd 端口*/
	uint64_t real_memory;	/* configured MB of real memory on the node 节点内存*/
	char *reason;		/* reason for node being DOWN or DRAINING DOWN和DRAINING原因*/
	time_t reason_time;	/* Time stamp when reason was set, ignore if设置原因时的时间戳
				 * no reason is set. */
	uint32_t reason_uid;   	/* User that set the reason, ignore if设置原因的用户
				 * no reason is set. */
	dynamic_plugin_data_t *select_nodeinfo;  /* opaque data structure,
						  * use
						  * slurm_get_select_nodeinfo()
						  * to access contents */
	time_t slurmd_start_time;/* time of slurmd startup slurmd启动时间*/
	uint16_t sockets;       /* total number of sockets per node 每个节点的sockets数量*/
	uint16_t threads;       /* number of threads per core 每个核心的线程数*/
	uint32_t tmp_disk;	/* configured MB of total disk in TMP_FS */
	uint32_t weight;	/* arbitrary priority of node for scheduling 调度时节点的随机权重*/
	char *tres_fmt_str;	/* str representing configured TRES on node */
	char *version;		 /* Slurm version number slurm软件版本号*/
} node_info_t;

typedef struct node_info_msg {
	time_t last_update;		/* time of latest info */
	uint32_t record_count;		/* number of records 记录的数量*/
	node_info_t *node_array;	/* the node records 节点记录*/
} node_info_msg_t;
...
typedef struct slurm_ctl_conf {
	time_t last_update;	/* 构建参数的最后更新时间 */
	char *accounting_storage_tres; /* tres链表 */
	...
	char *sched_params;	/* 调度器参数或调度器插件配置文件的内容 */
	...
	char *slurm_conf;	/* slurm配置文件的路径名 */
	uint32_t slurm_user_id;	/* uid of slurm_user_name */
	char *slurm_user_name;	/* slurmctld以哪个用户运行*/
	...
	uint32_t slurmctld_port;  /* 跟slurmctld的默认通讯端口 */
	uint16_t slurmctld_port_count; /* slurmctld通讯端口数量 */
	...
	uint32_t slurmd_port;	/* 跟slurmd的默认通讯端口 */
	...
	char *state_save_location;/* slurmctld状态保存目录的路径名spool */
